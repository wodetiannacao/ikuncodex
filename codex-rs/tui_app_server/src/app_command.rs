...
impl AppCommand {
    ...
    pub(crate) fn thread_rollback(num_turns: u32, scope: Option<BacktrackScope>) -> Self {
        match scope {
            Some(scope) => Self(Op::BacktrackRollback { num_turns, scope }),
            None => Self(Op::ThreadRollback { num_turns }),
        }
    }
    ...
    pub(crate) fn view(&self) -> AppCommandView<'_> {
        match &self.0 {
            ...
            Op::BacktrackRollback { num_turns, scope } => AppCommandView::ThreadRollback {
                num_turns: *num_turns,
                scope: Some(*scope),
            },
...
/*
   编号（如：1）：修改
   主要修改内容：扩展 ThreadRollback 视图携带可选 BacktrackScope，并支持映射 BacktrackRollback。
   修改目的：让 app-server TUI 能把对话回退、代码回退和组合回退统一传到底层协议。
