...
fn extend_lines_background(
    ...
    lines
        .into_iter()
        .map(|line| extend_line_background(line, target_width, style))
        .collect()
}
...
impl HistoryCell for UserHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let mut lines: Vec<Line<'static>> = vec![extend_line_background(
        ...
        if let Some(wrapped_remote_images) = wrapped_remote_images {
            lines.extend(extend_lines_background(
                prefix_lines(
            ...
            if wrapped_message.is_some() {
                lines.push(extend_line_background(
                    Line::from("").style(style),
                    target_width,
                    style,
                ));
        ...
        if let Some(wrapped_message) = wrapped_message {
            lines.extend(extend_lines_background(
                prefix_lines(
                    wrapped_message,
                    Span::styled("❯ ", style.fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                    Span::styled("  ", style),
                ),
                target_width,
                style,
...
impl HistoryCell for AgentMessageCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // 给助手文本块补一行轻量留白，减少不同消息块黏在一起的感觉。
        if self.is_first_line && !lines.is_empty() {
...
pub(crate) fn render_plan_update_lines(
    width: u16,
...
impl HistoryCell for PlanUpdateCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        render_plan_update_lines(width, self.explanation.as_deref(), &self.plan)
    }
}
...
mod tests {
    ...
    fn user_history_cell_height_matches_rendered_lines_with_remote_images() {
        ...
        let rendered_len: u16 = cell
            ...
            .unwrap_or(u16::MAX);
        assert!(
            cell.desired_height(width) >= rendered_len,
            "desired_height should account for wrapped remote image label rows"
        );
        assert_eq!(
...
/*
   编号（如：1）：修改
   主要修改内容：保留 Updated Plan 的共享渲染函数，并把仅供测试使用的计划历史单元类型限定到测试构建中。
   修改目的：继续复用统一的计划渲染样式，同时避免正式构建里出现不再使用的计划历史单元告警。
