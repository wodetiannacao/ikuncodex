...
use codex_app_server_protocol::BacktrackScope as ApiBacktrackScope;
        ...
        self.client
            .request_typed(ClientRequest::ThreadRollback {
                ...
                params: ThreadRollbackParams {
                    thread_id: thread_id.to_string(),
                    num_turns,
                    scope: scope.map(ApiBacktrackScope::from),
                },
            })
            .await
...
/*
   编号（如：1）：修改
   主要修改内容：thread_rollback RPC 新增可选 scope 参数，并转换为 app-server 协议字段。
   修改目的：让实际运行的 tui_app_server 可以把回退范围透传到 app-server。
