...
#[tokio::test]
async fn status_line_current_dir_uses_basename_only() {
    let (mut chat, _rx, _ops) = make_chatwidget_manual(None).await;
    chat.config.cwd = PathBuf::from("/tmp/demo/project-alpha").abs();

    assert_eq!(
        chat.status_line_value_for_item(&crate::bottom_pane::StatusLineItem::CurrentDir),
        Some("project-alpha".to_string())
    );
}

#[tokio::test]
async fn status_line_five_hour_limit_uses_remaining_label() {
    let (mut chat, _rx, _ops) = make_chatwidget_manual(None).await;
...
async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget {
        ...
        task_progress_phase: TaskProgressPhase::Planning,
...
async fn live_app_server_command_execution_strips_shell_wrapper() {
    ...
    assert!(
        cells.is_empty(),
        "completed commands now stay grouped in the active exec summary until a later boundary"
    );
...
async fn exec_history_cell_shows_working_then_completed() {
    ...
    assert!(
        cells.is_empty(),
        "completed exec cells should remain grouped until a later transcript boundary"
    );
...
async fn exec_history_cell_shows_working_then_failed() {
    ...
    assert!(
        ...
        "default compact mode should hide command output until Ctrl and O are pressed: {blob:?}"
    );
}

#[tokio::test]
async fn exec_lifecycle_resets_detail_toggle_back_to_compact() {
    let previous = crate::exec_cell::set_exec_output_details_expanded(true);
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    let begin = begin_exec(&mut chat, "call-compact-reset", "python -V");
    end_exec(
        &mut chat,
        begin,
        "python-version-visible-only-when-expanded",
        "",
        0,
    );
    ...
    assert!(
        blob.contains("Ran 1"),
        "expected collapsed summary headline to remain visible: {blob:?}"
    );
    assert!(
        blob.contains("python -V"),
        "expected compact summary headline to remain visible: {blob:?}"
    );
    assert!(
        !blob.contains("python-version-visible-only-when-expanded"),
        "new command lifecycles should force the default compact summary back on: {blob:?}"
    );
...
#[tokio::test]
async fn sequential_exec_calls_stay_grouped_in_one_active_summary() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    let first = begin_exec(&mut chat, "call-group-1", "python -V");
    end_exec(&mut chat, first, "Python 3.12\n", "", 0);
    assert!(
        drain_insert_history(&mut rx).is_empty(),
        "first completed exec call should remain in the active grouped block"
    );

    let second = begin_exec(&mut chat, "call-group-2", "cargo check");
    end_exec(&mut chat, second, "Finished\n", "", 0);
    assert!(
        drain_insert_history(&mut rx).is_empty(),
        "second completed exec call should remain grouped until a later boundary"
    );

    let blob = active_blob(&chat);
    assert!(
        blob.contains("Ran 2"),
        "expected grouped count in summary: {blob:?}"
    );
    assert!(
        blob.contains("python -V"),
        "expected first command preview: {blob:?}"
    );
    assert!(
        blob.contains("cargo check"),
        "expected second command preview: {blob:?}"
    );
}

#[tokio::test]
async fn ctrl_o_toggle_switches_exec_summary_between_aggregate_and_per_command_rows() {
    let previous = crate::exec_cell::set_exec_output_details_expanded(false);
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    let first = begin_exec(&mut chat, "call-toggle-1", "python -V");
    end_exec(&mut chat, first, "Python 3.12\n", "", 0);
    let second = begin_exec(&mut chat, "call-toggle-2", "cargo check");
    end_exec(&mut chat, second, "Finished\n", "", 0);

    assert!(
        drain_insert_history(&mut rx).is_empty(),
        "completed exec calls should stay in the active grouped block until a later boundary"
    );

    let compact_blob = active_blob(&chat);
    assert!(
        compact_blob.contains("Ran 2"),
        "default view should stay on the aggregate summary: {compact_blob:?}"
    );
    assert!(
        !compact_blob.contains("Ran python -V"),
        "default view should not expand into per-command rows yet: {compact_blob:?}"
    );

    let expanded = chat.toggle_exec_output_details();
    assert!(expanded, "Ctrl 和 O 切换后应进入逐条命令视图");

    let expanded_blob = active_blob(&chat);
    assert!(
        expanded_blob.contains("Ran python -V"),
        "expanded mode should show the first compact command row: {expanded_blob:?}"
    );
    assert!(
        expanded_blob.contains("Ran cargo check"),
        "expanded mode should show the second compact command row: {expanded_blob:?}"
    );
    assert!(
        !expanded_blob.contains("Python 3.12"),
        "expanded mode should still hide old multiline command output details: {expanded_blob:?}"
    );

    let collapsed_again = chat.toggle_exec_output_details();
    assert!(!collapsed_again, "再次按 Ctrl 和 O 后应回到聚合摘要视图");
    let collapsed_blob = active_blob(&chat);
    assert!(
        collapsed_blob.contains("Ran 2"),
        "second toggle should restore the aggregate summary: {collapsed_blob:?}"
    );
    assert!(
        !collapsed_blob.contains("Ran python -V"),
        "second toggle should hide the per-command rows again: {collapsed_blob:?}"
    );
...
async fn exec_end_without_begin_uses_event_command() {
    ...
    assert!(
        cells.is_empty(),
        "orphan exec end should stay in the active compact command group until a later boundary"
    );
    let blob = active_blob(&chat);
    assert!(
        blob.contains("Ran 1"),
    ...
    assert!(
        blob.contains("echo orphaned") && !blob.contains("call-orphan"),
        "expected rendered summary to use the event command rather than the call id: {blob:?}"
    );
}
...
async fn exec_end_without_begin_flushes_completed_unrelated_exploring_cell() {
    ...
    assert_eq!(
        cells.len(),
        1,
        "completed exploring cell should flush before the orphan entry becomes the new active block"
    );
    ...
    let active = active_blob(&chat);
    assert!(
        active.contains("echo after"),
        "expected orphan end entry to stay active after flushing the previous exploring block: {active:?}"
    );
    assert!(
        chat.active_cell.is_some(),
        "orphan end should remain active for grouping"
    );
}
...
async fn unified_exec_wait_status_header_updates_on_late_command_display() {
    ...
    let details = status
        .details()
        .expect("waiting status should include details");
    assert!(details.contains("sleep 5"));
    assert!(!details.contains("理解需求与规划"));
}
...
async fn unified_exec_waiting_multiple_empty_snapshots() {
    ...
    let details = status
        .details()
        .expect("waiting status should include the command");
    assert!(details.contains("just fix"));
    assert!(!details.contains("理解需求与规划"));
...
async fn user_shell_command_renders_output_not_exploring() {
    ...
    assert!(
        cells.is_empty(),
        "user shell commands now stay in the grouped active exec summary until a later boundary"
    );
    let blob = active_blob(&chat);
...
async fn plan_update_renders_history_cell() {
    ...
    assert!(
        cells.is_empty(),
        "persistent Updated Plan should not be duplicated into transcript history: {cells:?}"
    );
    let persistent = render_bottom_popup(&chat, 80);
    assert!(persistent.contains("Updated Plan"));
    assert!(persistent.contains("Explore codebase"));
    assert!(persistent.contains("Implement feature"));
    assert!(persistent.contains("Write tests"));
}
...
async fn stream_error_updates_status_indicator() {
    ...
    let rendered_details = status
        .details()
        .expect("stream retry details should include the retry reason");
    assert!(rendered_details.contains(details));
    assert!(!rendered_details.contains("理解需求与规划"));
}
...
async fn status_line_model_with_reasoning_includes_fast_for_gpt54_only() {
    ...
    assert_eq!(
        ...
        Some("gpt-5.4 xhigh fast · 100% left · project".to_string())
    );
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("gpt-5.3-codex xhigh · 100% left · project".to_string())
    );
}
...
/*
   编号（如：1）：修改
   主要修改内容：更新了计划更新与底栏额度相关测试，改为校验常驻 Updated Plan 和新的 5h/7d 紧凑状态格式。
   修改目的：让测试与当前产品行为保持一致，防止历史计划块和旧底栏文案再次回归。
