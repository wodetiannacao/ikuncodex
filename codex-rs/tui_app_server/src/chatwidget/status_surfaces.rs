    ...
    pub(super) fn status_line_value_for_item(&mut self, item: &StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::FiveHourLimit => {
                ...
                self.status_line_limit_display(window, &compact_limit_label(&label))
            }
...
/*
   编号（如：1）：修改
   主要修改内容：将底栏额度标签压缩为 5h/7d 这类短格式，并为周/月窗口提供统一的紧凑标签映射。
   修改目的：让底栏在有限宽度里同时容纳模型、额度百分比和刷新倒计时，而不显得拥挤。
