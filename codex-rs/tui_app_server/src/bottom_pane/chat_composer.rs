...
impl ChatComposer {
    pub(crate) fn render_with_mask(&self, area: Rect, buf: &mut Buffer, mask_char: Option<char>) {
        ...
        // Keep the live composer visually neutral so only committed user messages
        // carry the gray "sent message" background in the transcript.
...
/*
   编号（如：1）：修改
   主要修改内容：恢复底部输入框为默认中性样式，不再复用用户消息的灰底背景。
   修改目的：让真正发出去的用户消息继续高亮，而正在输入的草稿保持和官方输入框一致。
