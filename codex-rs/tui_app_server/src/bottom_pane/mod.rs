...
pub(crate) struct BottomPane {
    ...
    /// Latest official Updated Plan block kept visible above the composer.
    persistent_plan: Option<UpdatePlanArgs>,
...
impl BottomPane {
    ...
    pub(crate) fn set_persistent_plan(&mut self, plan: Option<UpdatePlanArgs>) {
        self.persistent_plan = plan;
        self.request_redraw();
    }
    ...
    fn as_renderable(&'_ self) -> RenderableItem<'_> {
        if let Some(view) = self.active_view() {
            ...
            if let Some(plan) = &self.persistent_plan {
                let has_above_content = self.status.is_some()
                    || !self.unified_exec_footer.is_empty()
                    || has_pending_thread_approvals
                    || has_pending_input;
                if has_above_content {
                    flex2.push(/*flex*/ 0, RenderableItem::Owned("".into()));
                }
                flex2.push(
                    /*flex*/ 0,
                    RenderableItem::Owned(Box::new(PersistentPlanRenderable {
                        update: plan.clone(),
                    })),
                );
                flex2.push(/*flex*/ 0, RenderableItem::Owned("".into()));
...
#[derive(Debug)]
struct PersistentPlanRenderable {
    update: UpdatePlanArgs,
}

impl Renderable for PersistentPlanRenderable {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        let lines = history_cell::render_plan_update_lines(
            area.width,
            self.update.explanation.as_deref(),
            &self.update.plan,
        );
...
mod tests {
    ...
    #[test]
    fn persistent_plan_renders_above_composer() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventSender::new(tx_raw);
        let mut pane = BottomPane::new(BottomPaneParams {
            app_event_tx: tx,
        ...
        pane.set_persistent_plan(Some(UpdatePlanArgs {
            explanation: Some("整理当前任务并保持计划常驻。".to_string()),
            plan: vec![
                codex_protocol::plan_tool::PlanItemArg {
                    step: "整理输出区域".to_string(),
                    status: codex_protocol::plan_tool::StepStatus::Completed,
                },
                codex_protocol::plan_tool::PlanItemArg {
                    step: "验证常驻计划区域".to_string(),
                    status: codex_protocol::plan_tool::StepStatus::InProgress,
                },
            ],
        }));

        let snapshot = render_snapshot(&pane, Rect::new(0, 0, 48, 10));
        assert!(snapshot.contains("Updated Plan"));
        assert!(snapshot.contains("Ask Codex to do anything"));
    }
}

/*
   编号（如：1）：修改
   主要修改内容：在输入框上方新增常驻 Updated Plan 区块，并复用官方计划渲染样式显示最新步骤。
   修改目的：让计划信息持续可见，同时避免遮挡上方对话内容。
