...
impl Default for MarkdownStyles {
    fn default() -> Self {
        ...
        Self {
            h1: Style::new().bold().underlined().light_cyan(),
            h2: Style::new().bold().cyan(),
            h3: Style::new().bold().italic().light_blue(),
            h4: Style::new().italic().yellow(),
            h5: Style::new().italic().light_yellow(),
            h6: Style::new().italic().gray(),
            code: Style::new().cyan(),
    ...
    I: Iterator<Item = Event<'a>>,
{
    ...
    fn flush_current_line(&mut self) {
        if let Some(line) = self.current_line_content.take() {
            let style = self
                .current_line_style
                .patch(semantic_line_style(&line).unwrap_or_default());
...
fn semantic_line_style(line: &Line<'static>) -> Option<Style> {
    use ratatui::style::Stylize;
    ...
    if trimmed.starts_with("🔴") || trimmed.starts_with("高危") {
        Some(Style::new().bold().red())
    } else if trimmed.starts_with("🟡") || trimmed.starts_with("中等") {
        Some(Style::new().bold().yellow())
    } else if trimmed.starts_with("🟢") || trimmed.starts_with("轻微") {
...
/*
   编号（如：1）：修改
   主要修改内容：增强了 Markdown 标题层级颜色，并为 🔴/🟡/🟢 与高危/中等/轻微 这类审查标记增加固定语义着色。
   修改目的：让终端里的重点标题和风险结论更醒目，同时保持整体样式不过度花哨。
