...
//
// 编号（如：1）：修改
// 主要修改内容：将模型选择与会话恢复相关的运行时提示命令统一改为 ikuncodex。
// 修改目的：保证当前实际 UI 链路里的用户指引与新全局命令保持一致，避免出现 command not found。
//
...
const DEFAULT_STATUS_LINE_ITEMS: [&str; 3] =
    ["model-with-reasoning", "five-hour-limit", "current-dir"];
...
pub(crate) struct ChatWidget {
    ...
    // Latest explicit plan steps for live task-progress rendering.
    last_plan_items: Vec<UpdatePlanItemArg>,
    // Heuristic task phase used when the runtime did not provide explicit plan steps.
    task_progress_phase: TaskProgressPhase,
...
impl ChatWidget {
    ...
    fn set_status(
        ...
        let rendered_details = raw_details.clone();
        self.current_status = StatusIndicatorState {
            header: header.clone(),
            details: raw_details.clone(),
            details_max_lines,
        };
    ...
    fn on_task_started(&mut self) {
        ...
        self.task_progress_phase = TaskProgressPhase::Planning;
        self.plan_delta_buffer.clear();
        self.plan_item_active = false;
        self.adaptive_chunking.reset();
        ...
        self.terminal_title_status_kind = TerminalTitleStatusKind::Working;
        self.refresh_live_task_progress();
        self.set_status_header(String::from("Working"));
        self.full_reasoning_buffer.clear();
        self.reasoning_buffer.clear();
        self.request_redraw();
    ...
    fn on_plan_update(&mut self, update: UpdatePlanArgs) {
        ...
        self.bottom_pane.set_persistent_plan(Some(update.clone()));
        ...
        self.refresh_terminal_title();
        // The persistent plan panel now owns plan visibility so the transcript stays cleaner.
        self.refresh_live_task_progress();
    }
    ...
    fn on_exec_command_begin(&mut self, ev: ExecCommandBeginEvent) {
        self.flush_answer_stream_with_separator();
        self.task_progress_phase = if matches!(self.task_progress_phase, TaskProgressPhase::Editing)
        {
            TaskProgressPhase::Verifying
        } else {
            TaskProgressPhase::Inspecting
        };
        self.refresh_live_task_progress();
    ...
    fn on_patch_apply_begin(&mut self, event: PatchApplyBeginEvent) {
        self.task_progress_phase = TaskProgressPhase::Editing;
        self.refresh_live_task_progress();
    ...
    pub(crate) fn handle_exec_end_now(&mut self, ev: ExecCommandEndEvent) {
        // Expanded exec details are only a temporary inspection mode. Once the
        // command lifecycle advances, restore the default compact summary.
        let _was_expanded = crate::exec_cell::collapse_exec_output_details();
        enum ExecEndTarget {
        ...
        match end_target {
            ExecEndTarget::ActiveTracked => {
                    ...
                    .and_then(|c| c.as_any_mut().downcast_mut::<ExecCell>())
                {
                    ...
                    // Keep sequential exec calls grouped in one visible block until a later
                    // transcript boundary (assistant text, MCP output, end-of-turn separator, etc.)
                    // flushes the active cell into committed history.
                    self.needs_final_message_separator = true;
                    self.bump_active_cell_revision();
                    self.request_redraw();
                }
    ...
    pub(crate) fn handle_patch_apply_end_now(
        ...
        self.had_work_activity = true;
        self.task_progress_phase = TaskProgressPhase::Verifying;
        self.refresh_live_task_progress();
    }
    ...
    pub(crate) fn handle_exec_begin_now(&mut self, ev: ExecCommandBeginEvent) {
        ...
        self.bottom_pane.ensure_status_indicator();
        // Every new command starts in the compact single-line summary view.
        let _was_expanded = crate::exec_cell::collapse_exec_output_details();
    ...
    fn status_line_limit_display(
        ...
        match window.resets_in_compact.as_deref() {
            Some(resets_in) if !resets_in.is_empty() => {
                Some(format!("{label} {remaining:.0}% {resets_in}"))
            }
            _ => Some(format!("{label} {remaining:.0}%")),
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running ikuncodex -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = codex_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("ikuncodex resume {name}"));
    ...
    pub(crate) fn toggle_exec_output_details(&mut self) -> bool {
        let expanded = crate::exec_cell::toggle_exec_output_details();
        if self
            .active_cell
            .as_ref()
            .is_some_and(|cell| cell.as_any().is::<crate::exec_cell::ExecCell>())
        {
            self.bump_active_cell_revision();
        }
        self.request_redraw();
    ...
    pub(crate) fn toggle_diff_output_details(&mut self) -> bool {
        let expanded = crate::diff_render::toggle_history_diff_details();
        self.request_redraw();
        expanded
    }

    fn refresh_live_task_progress(&mut self) {
        if !self.bottom_pane.is_task_running() {
            return;
        }
        ...
        self.set_status(
            ...
            self.current_status.details.clone(),
            StatusDetailsCapitalization::Preserve,
            self.current_status.details_max_lines.max(6),
        );
    }
...
/*
   编号（如：1）：修改
   主要修改内容：去掉了对话历史里的 Updated Plan 注入，只保留输入框上方的常驻计划区；同时补充底栏额度的紧凑倒计时显示。
   修改目的：让计划区固定且不重复出现，并让 5h/7d 状态一眼看出剩余百分比与刷新时间。
