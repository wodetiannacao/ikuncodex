...
//
// 编号（如：1）：修改
// 主要修改内容：将 app-server 语音鉴权失败提示中的登录命令统一为 ikuncodex login。
// 修改目的：保证当前主链路里的用户恢复步骤与新发布命令保持一致。
//
...
async fn resolve_auth() -> Result<TranscriptionAuthContext, String> {
    ...
    let auth = CodexAuth::from_auth_storage(&codex_home, AuthCredentialsStoreMode::Auto)
        ...
        .ok_or_else(|| "No Codex auth is configured; please run `ikuncodex login`".to_string())?;
