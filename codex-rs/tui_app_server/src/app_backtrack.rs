...
impl App {
    ...
    pub(crate) fn apply_backtrack_rollback(
        ...
        if scope.includes_conversation() && self.backtrack.pending_rollback.is_some() {
            self.chat_widget
        ...
        self.chat_widget
            .submit_op(AppCommand::thread_rollback(num_turns, Some(scope)));
        if scope.includes_conversation() {
    ...
    fn open_backtrack_preview(&mut self, tui: &mut tui::Tui) {
        self.open_backtrack_prompt_picker(tui);
        self.reset_backtrack_state();
    }
    ...
    pub(crate) fn apply_backtrack_selection(
        ...
        self.apply_backtrack_rollback(selection, scope);
        tui.frame_requester().schedule_frame();
    }

    pub(crate) fn open_backtrack_scope_picker(
        &mut self,
        tui: &mut tui::Tui,
        selection: BacktrackSelection,
    ) {
        let items = vec![
            backtrack_scope_item(
                "仅回退对话",
                Some("只回退消息历史，保留当前代码改动。".to_string()),
                selection.clone(),
                BacktrackScope::ConversationOnly,
            ),
            backtrack_scope_item(
                "仅回退代码",
                Some("只回退 Codex 记录到的代码改动，不改动对话历史。".to_string()),
                selection.clone(),
                BacktrackScope::FilesOnly,
            ),
            backtrack_scope_item(
                "对话和代码一起回退",
                Some("同时回退消息历史与本轮之后的代码改动。".to_string()),
                selection,
                BacktrackScope::ConversationAndFiles,
            ),
        ];

        self.chat_widget.show_selection_view(SelectionViewParams {
            title: Some("选择回退范围".to_string()),
            subtitle: Some(
                "Esc-Esc 选中历史消息后，可以单独回退对话、代码，或一起回退。".to_string(),
            ),
            items,
    ...
    fn open_backtrack_prompt_picker(&mut self, tui: &mut tui::Tui) {
        let items = self.backtrack_prompt_items();
        self.chat_widget.clear_esc_backtrack_hint();
        if items.is_empty() {
            tui.frame_requester().schedule_frame();
            return;
        ...
        self.chat_widget.show_selection_view(SelectionViewParams {
            title: Some("选择要回退到的提示词".to_string()),
            subtitle: Some(
                "这里只显示你发过的提示词。选中后，再选择仅回退对话、仅回退代码，或一起回退。"
                    .to_string(),
            ),
            items,
    ...
    fn backtrack_prompt_items(&self) -> Vec<SelectionItem> {
        let total_users = user_count(&self.transcript_cells);
        (0..total_users)
            .rev()
            .filter_map(|nth_user_message| {
                let selection = self.backtrack_selection(nth_user_message)?;
                let prompt_summary = summarize_backtrack_prompt(&selection.prefill);
                Some(SelectionItem {
                    name: prompt_summary.clone(),
                    search_value: Some(prompt_summary),
                    dismiss_on_select: true,
                    actions: vec![Box::new(move |tx| {
                        tx.send(AppEvent::OpenBacktrackScopePicker {
                            selection: selection.clone(),
                        });
                    })],
                    ..Default::default()
...
fn summarize_backtrack_prompt(prompt: &str) -> String {
    let single_line = prompt
        .lines()
        .map(str::trim)
        .find(|line| !line.is_empty())
        .unwrap_or("(空提示词)");
    truncate_backtrack_prompt(single_line, 72)
}

fn truncate_backtrack_prompt(text: &str, max_chars: usize) -> String {
...
mod tests {
    ...
    fn trim_transcript_preserves_cells_before_selected_user() {
        ...
        assert!(
            !agent_lines.is_empty(),
            "expected preserved intro message to keep at least one rendered line"
        );
...
/*
   编号（如：1）：修改
   主要修改内容：撤销 transcript overlay 内对 Ctrl+O 和 Ctrl+M 的特殊拦截，恢复 overlay 的原始事件转发行为。
   修改目的：回退上一轮 detail toggle 的交互变化，保持回退预览流程与主界面行为一致。
