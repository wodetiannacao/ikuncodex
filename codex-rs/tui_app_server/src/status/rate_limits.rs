...
pub(crate) struct RateLimitWindowDisplay {
    ...
    /// Compact countdown text used by the status line, such as `01:42` or `2d4h`.
    pub resets_in_compact: Option<String>,
...
impl RateLimitWindowDisplay {
    fn from_window(window: &RateLimitWindow, captured_at: DateTime<Local>) -> Self {
        ...
        let resets_in_compact =
            resets_at_utc.and_then(|dt| format_compact_reset_delta(dt, captured_at));
...
fn format_compact_reset_delta(
    reset_at: DateTime<Local>,
    captured_at: DateTime<Local>,
) -> Option<String> {
    let delta = reset_at.signed_duration_since(captured_at);
...
mod tests {
    ...
    fn window(used_percent: f64) -> RateLimitWindowDisplay {
        RateLimitWindowDisplay {
            ...
            resets_in_compact: Some("00:45".to_string()),
    ...
    fn non_codex_multi_limit_keeps_group_row() {
        ...
        let other = RateLimitSnapshotDisplay {
            ...
            secondary: Some(RateLimitWindowDisplay {
                ...
                resets_in_compact: Some("2d4h".to_string()),
                window_minutes: None,
            }),
            credits: None,
        };
...
/*
   编号（如：1）：修改
   主要修改内容：为 rate limit 展示结构补充了紧凑倒计时字段，用于底栏显示 5h/7d 剩余刷新时间。
   修改目的：让底栏能同时显示额度剩余百分比和距离刷新还剩多久，而不必重复计算时间格式。
