...
impl App {
    ...
    fn transcript_history_lines(
        &self,
        width: u16,
        has_existing_history: bool,
    ) -> Vec<Line<'static>> {
        let mut emitted_any_history = has_existing_history;
        ...
        for cell in &self.transcript_cells {
            ...
            if !cell.is_stream_continuation() {
                if emitted_any_history {
                    display.insert(0, Line::from(""));
                } else {
                    emitted_any_history = true;
                }
            }
    ...
    fn refresh_transcript_history_after_exec_toggle(&mut self, tui: &mut tui::Tui) {
        let width = tui.terminal.last_known_screen_size.width;
        if let Some(Overlay::Transcript(overlay)) = &mut self.overlay {
            overlay.replace_cells(self.transcript_cells.clone());
            self.deferred_history_lines =
                self.transcript_history_lines(width, self.has_emitted_history_lines);
            return;
        }

        let viewport_anchor = self.transcript_viewport_anchor(tui);
        if let Err(err) = self.rebuild_transcript_history_in_place(tui, width, viewport_anchor) {
            tracing::warn!(error = %err, "failed to rebuild transcript after Ctrl+O toggle");
            self.chat_widget
                .add_error_message(format!("Failed to refresh command transcript: {err}"));
        }
    }

    fn refresh_transcript_history_after_diff_toggle(&mut self, tui: &mut tui::Tui) {
        let width = tui.terminal.last_known_screen_size.width;
        if let Some(Overlay::Transcript(overlay)) = &mut self.overlay {
            overlay.replace_cells(self.transcript_cells.clone());
            self.deferred_history_lines =
        ...
        let viewport_anchor = self.transcript_viewport_anchor(tui);
        if let Err(err) = self.rebuild_transcript_history_in_place(tui, width, viewport_anchor) {
            tracing::warn!(error = %err, "failed to rebuild transcript after Ctrl+M toggle");
            self.chat_widget
                .add_error_message(format!("Failed to refresh patch transcript: {err}"));
        }
    }

    fn transcript_viewport_anchor(&self, tui: &tui::Tui) -> Option<u16> {
        self.has_emitted_history_lines
            .then_some(tui.terminal.viewport_area.y)
    }

    fn rebuild_transcript_history_in_place(
        &mut self,
        tui: &mut tui::Tui,
        width: u16,
        viewport_anchor: Option<u16>,
    ) -> Result<()> {
        // Rebuild directly into the terminal so we can preserve the user's current scroll anchor
        // after toggling compact/expanded transcript views.
        self.clear_terminal_ui(tui, /*redraw_header*/ false)?;

        let mut has_existing_history = false;
        let header_lines = self.clear_ui_header_lines(width);
        if !header_lines.is_empty() {
            crate::insert_history::insert_history_lines(&mut tui.terminal, header_lines)?;
            has_existing_history = true;
        }

        let lines = self.transcript_history_lines(width, has_existing_history);
        if !lines.is_empty() {
            crate::insert_history::insert_history_lines(&mut tui.terminal, lines)?;
            has_existing_history = true;
        }
        self.has_emitted_history_lines = has_existing_history;

        if let Some(anchor_y) = viewport_anchor {
            let mut area = tui.terminal.viewport_area;
            // Clamp to the rebuilt transcript height so toggling never jumps to the newest lines.
            area.y = anchor_y.min(area.y);
    ...
    async fn try_submit_active_thread_op_via_app_server(
        ...
        match op.view() {
            ...
            AppCommandView::ThreadRollback { num_turns, scope } => {
                let response = match app_server
                    .thread_rollback(thread_id, num_turns, scope)
                    .await
                {
    ...
    async fn handle_event(
        ...
        match event {
            ...
            AppEvent::ApplyBacktrackSelection { selection, scope } => {
                self.apply_backtrack_selection(tui, selection, scope);
            }
            AppEvent::OpenBacktrackScopePicker { selection } => {
                self.open_backtrack_scope_picker(tui, selection);
            }
    ...
    async fn handle_key_event(
        ...
        match key_event {
            ...
            KeyEvent {
                ...
                let _expanded = self.chat_widget.toggle_exec_output_details();
                self.refresh_transcript_history_after_exec_toggle(tui);
                tui.frame_requester().schedule_frame();
            }
            KeyEvent {
                code: KeyCode::Char('m'),
                ...
                let _expanded = self.chat_widget.toggle_diff_output_details();
                self.refresh_transcript_history_after_diff_toggle(tui);
                tui.frame_requester().schedule_frame();
            }
            KeyEvent {
                code: KeyCode::Char('l'),
...
mod tests {
    ...
    use crate::exec_cell::set_exec_output_details_expanded;
...
"#;
    ...
    async fn model_migration_prompt_shows_for_hidden_model() {
        ...
        let current_index = available_models
            .iter()
            .position(|preset| preset.upgrade.is_some())
            .expect("expected at least one model preset with an upgrade target");
        available_models[current_index].show_in_picker = false;
        let current = available_models[current_index].clone();
    ...
    async fn backtrack_selection_with_duplicate_history_targets_unique_turn() {
        ...
        app.apply_backtrack_rollback(
            selection,
            codex_protocol::protocol::BacktrackScope::ConversationAndFiles,
        );
        assert_eq!(
        ...
        while let Ok(op) = op_rx.try_recv() {
            if let Op::BacktrackRollback { num_turns, .. } = op {
    ...
    async fn backtrack_resubmit_preserves_data_image_urls_in_user_turn() {
        ...
        while let Ok(op) = op_rx.try_recv() {
            match op {
                Op::BacktrackRollback { .. } => saw_rollback = true,
    ...
    #[tokio::test]
    async fn transcript_history_lines_follow_global_exec_detail_toggle() {
        let previous = set_exec_output_details_expanded(false);

        let call_id = "exec-toggle".to_string();
        let mut exec_cell = ExecCell::new(
            ExecCall {
        ...
        exec_cell.complete_call(
            ...
            CommandOutput {
                ...
                aggregated_output: "README.md".to_string(),
            },
            Duration::from_millis(15),
        );

        let mut app = make_test_app().await;
        app.transcript_cells = vec![Arc::new(exec_cell) as Arc<dyn HistoryCell>];

        let compact = app
            .transcript_history_lines(100, /*has_existing_history*/ true)
            .iter()
            .map(|line| {
                line.spans
                    .iter()
            ...
            .join("\n");
        assert!(
            compact.contains("Ran 1")
                && compact.contains("commands")
                && compact.contains("Get-ChildItem -Force"),
            "collapsed transcript should keep the aggregate summary: {compact:?}"
        );
        assert!(
            !compact.contains("Ran Get-ChildItem -Force"),
            "collapsed transcript should hide per-command rows: {compact:?}"
        );

        let _ = set_exec_output_details_expanded(true);
        let expanded = app
            .transcript_history_lines(100, /*has_existing_history*/ true)
            .iter()
            .map(|line| {
            ...
            .join("\n");
        assert!(
            expanded.contains("Ran powershell -NoProfile")
                && expanded.contains("Get-ChildItem -Force"),
            "expanded transcript should show the per-command row: {expanded:?}"
        );

        let _ = set_exec_output_details_expanded(previous);
    }
    ...
    async fn session_summary_includes_resume_hint() {
        ...
        assert_eq!(
            ...
            Some("ikuncodex resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    async fn session_summary_prefers_name_over_id() {
        ...
        assert_eq!(
            summary.resume_command,
            Some("ikuncodex resume my-session".to_string())
        );
    }
}
...
/*
   编号（如：1）：修改
   主要修改内容：Ctrl+O 和 Ctrl+M 切换 transcript 详情时恢复为原地重建并保留当前滚动锚点，撤销 overlay 方案。
   修改目的：回退上一轮交互变化，恢复到更接近既有使用预期的切换方式。
