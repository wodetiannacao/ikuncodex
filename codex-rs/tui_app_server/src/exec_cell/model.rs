...
impl ExecCell {
    ...
    pub(crate) fn with_added_call(
        ...
        } else if !self.is_exploring_cell() && !Self::is_exploring_call(&call) {
            Some(Self {
                calls: [self.calls.clone(), vec![call]].concat(),
                animations_enabled: self.animations_enabled,
            })
    ...
    pub(crate) fn should_flush(&self) -> bool {
        // Keep completed exec groups in the active slot until a later transcript
        // boundary (assistant text, separator, tool result, etc.) flushes them.
        //
        // This lets the default compact view aggregate several consecutive
        // commands into a single summary block, while Ctrl+O can still expand
        // that same block into one-command-per-line details.
        false
    }
...
/*
   编号（如：1）：修改
   主要修改内容：调整 ExecCell 的刷新策略，让已完成命令组继续保留在活动区，等待后续边界统一落历史。
   修改目的：保证默认命令区可以稳定聚合多条命令摘要，并让 Ctrl+O 展开的逐条命令列表与聚合块保持一致。
