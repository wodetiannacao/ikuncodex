...
fn headings() {
    ...
    let expected = Text::from_iter([
        Line::from_iter([
            "# ".bold().underlined().light_cyan(),
            "Heading 1".bold().underlined().light_cyan(),
        ]),
        Line::default(),
        Line::from_iter(["## ".bold().cyan(), "Heading 2".bold().cyan()]),
        Line::default(),
        Line::from_iter([
            "### ".bold().italic().light_blue(),
            "Heading 3".bold().italic().light_blue(),
        ]),
        Line::default(),
        Line::from_iter(["#### ".italic().yellow(), "Heading 4".italic().yellow()]),
        Line::default(),
        Line::from_iter([
            "##### ".italic().light_yellow(),
            "Heading 5".italic().light_yellow(),
        ]),
        Line::default(),
        Line::from_iter(["###### ".italic().gray(), "Heading 6".italic().gray()]),
    ]);
    assert_eq!(text, expected);
}

#[test]
fn semantic_risk_markers_use_fixed_colors() {
    let text = render_markdown_text("🔴 高危：会阻塞运行\n🟡 中等：可能影响体验\n🟢 轻微：只是样式问题");
    assert_eq!(text.lines[0].style.fg, Some(ratatui::style::Color::Red));
    assert_eq!(text.lines[1].style.fg, Some(ratatui::style::Color::Yellow));
    assert_eq!(text.lines[2].style.fg, Some(ratatui::style::Color::Green));
}
    ...
    assert_eq!(
        ...
        [
            Line::from_iter([
                "> ".into(),
                "# ".bold().underlined().light_cyan(),
                "test header".bold().underlined().light_cyan(),
            ])
            .green(),
            Line::from_iter(["> "]).green(),
...
/*
   编号（如：1）：修改
   主要修改内容：同步更新了标题配色与风险标记语义着色的断言，覆盖块引用标题继承样式的场景。
   修改目的：确保新的 Markdown 重点高亮规则在测试里被稳定约束，不会悄悄退回到旧样式。
