...
/// Whether history patch blocks should render their full unified diff body.
///
/// The main chat transcript keeps code changes collapsed by default so turns
/// with many edits stay scannable. `Ctrl+M` flips this flag for the current
/// session and the app rebuilds transcript history so existing patch cells
/// refresh together.
#[cfg(not(test))]
static HISTORY_DIFF_DETAILS_EXPANDED: AtomicBool = AtomicBool::new(false);
#[cfg(test)]
static HISTORY_DIFF_DETAILS_EXPANDED: AtomicBool = AtomicBool::new(true);
...
pub(crate) fn history_diff_details_expanded() -> bool {
    HISTORY_DIFF_DETAILS_EXPANDED.load(Ordering::Relaxed)
}

pub(crate) fn toggle_history_diff_details() -> bool {
    let previous = HISTORY_DIFF_DETAILS_EXPANDED.fetch_xor(true, Ordering::Relaxed);
    !previous
}

#[cfg(test)]
pub(crate) fn set_history_diff_details_expanded(expanded: bool) -> bool {
    HISTORY_DIFF_DETAILS_EXPANDED.swap(expanded, Ordering::Relaxed)
}
...
pub(crate) fn create_diff_summary(
    ...
    if history_diff_details_expanded() {
        render_changes_block(rows, wrap_cols, cwd)
    } else {
        render_changes_summary_block(rows, cwd)
    }
}
...
fn render_changes_summary_block(rows: Vec<Row>, cwd: &Path) -> Vec<RtLine<'static>> {
    let mut out: Vec<RtLine<'static>> = Vec::new();
...
mod tests {
    ...
    struct HistoryDiffDetailsGuard {
        previous: bool,
    }

    impl HistoryDiffDetailsGuard {
        fn set(expanded: bool) -> Self {
            Self {
                previous: set_history_diff_details_expanded(expanded),
            }
        }
    }

    impl Drop for HistoryDiffDetailsGuard {
        fn drop(&mut self) {
            let _ = set_history_diff_details_expanded(self.previous);
        }
    }
    ...
    #[test]
    fn collapsed_history_diff_shows_only_file_summary() {
        let _guard = HistoryDiffDetailsGuard::set(false);
        let mut changes: HashMap<PathBuf, FileChange> = HashMap::new();
        changes.insert(
            PathBuf::from("new_file.txt"),
            FileChange::Add {
        ...
        assert_eq!(rendered, vec!["• Added new_file.txt (+2 -0)".to_string()]);
    }
...
/*
   编号（如：1）：修改
   主要修改内容：为历史区代码改动新增默认折叠模式，只显示文件名与增删行数，并提供全局展开状态开关。
   修改目的：让补丁历史默认更紧凑，同时保留按快捷键查看完整 diff 的能力。
