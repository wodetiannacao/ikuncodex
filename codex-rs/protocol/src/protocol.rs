...
pub enum Op {
    ...
    /// Request Codex to backtrack by user turns and optionally revert files
    /// captured by the non-Git checkpoint tracker.
    BacktrackRollback {
        num_turns: u32,
...
impl BacktrackScope {
    pub fn includes_conversation(self) -> bool {
        matches!(self, Self::ConversationOnly | Self::ConversationAndFiles)
    }

    pub fn includes_files(self) -> bool {
        matches!(self, Self::FilesOnly | Self::ConversationAndFiles)
    }
}
...
impl Op {
    pub fn kind(&self) -> &'static str {
        match self {
            ...
            Self::BacktrackRollback { .. } => "backtrack_rollback",
            Self::Review { .. } => "review",
...
mod tests {
    ...
    #[test]
    fn backtrack_rollback_serializes_scope_flattened() -> Result<()> {
        let op = Op::BacktrackRollback {
            num_turns: 2,
            scope: BacktrackScope::ConversationAndFiles,
        };
...
// 编号（如：1）：修改
// 主要修改内容：新增 BacktrackScope 与 backtrack_rollback 协议操作，并补充序列化测试。
// 修改目的：让 TUI 和 core 能明确区分“仅对话 / 仅代码 / 对话加代码”三种回退范围。
