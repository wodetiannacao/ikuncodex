...
impl App {
    ...
    pub(crate) fn apply_backtrack_rollback(
        ...
        self.chat_widget
            .submit_op(Op::BacktrackRollback { num_turns, scope });
        if scope.includes_conversation() {
    ...
    fn overlay_forward_event(&mut self, tui: &mut tui::Tui, event: TuiEvent) -> Result<()> {
        ...
        if let Some(overlay) = &mut self.overlay {
            ...
            if overlay.is_done() {
                ...
                // 如果是 Translate overlay，在关闭前取出配置并应用
    ...
    pub(crate) fn handle_backtrack_event(&mut self, event: &EventMsg) {
        match event {
            EventMsg::ThreadRolledBack(rollback) => {
                // `pending_rollback` is set only after this UI sends a backtrack rollback
...
// 编号（如：1）：修改
// 主要修改内容：为 Esc-Esc 回退流程新增范围选择弹窗，并把提交操作切换为 backtrack_rollback。
// 修改目的：支持“仅回退对话 / 仅回退代码 / 对话和代码一起回退”三种交互模式。
