...
"#;
    ...
    #[tokio::test]
    #[ignore = "cometix: all non-gpt-oss models are visible, hidden model migration scenario does not apply"]
...
// 编号（如：1）：修改
// 主要修改内容：主应用层新增 ApplyBacktrackSelection 处理，并将主界面 Enter 确认改为先弹回退范围选择。
// 修改目的：把 Esc-Esc 的消息选中流程升级为“先选消息，再选回退范围”的两段式交互。
// 编号（如：2）：修改
// 主要修改内容：新增 Ctrl+O 切换 Exec 详情展示，并将主聊天区非流式单元的垂直间距略微增大。
// 修改目的：默认保持输出简洁，同时提供随时查看详细命令状态的入口，并改善正文阅读节奏。
