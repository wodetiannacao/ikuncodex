...
//
// 编号（如：1）：修改
// 主要修改内容：将状态卡里 API key 登录提示从 codex login 改为 ikuncodex login。
// 修改目的：让账户状态区给出的下一步操作可直接在新 npm 包环境中执行。
//
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run ikuncodex login to use ChatGPT)".to_string()
            }
        });
