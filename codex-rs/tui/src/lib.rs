...
//
// 编号（如：1）：修改
// 主要修改内容：将 TUI 会话恢复/分叉失败时给用户的 CLI 指令提示统一改为 ikuncodex。
// 修改目的：避免用户按照错误的旧命令重试，保证 npm 改名后运行时提示仍然可执行。
//
mod bottom_pane;
mod chatwidget;
mod cli;
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `ikuncodex {action}` without an ID to choose from existing sessions."
            )),
        })
    };
                        ...
                        .await
                        {
                            ...
                            None => {
                                ...
                                return Ok(AppExitInfo {
                                    ...
                                    update_action: None,
                                    exit_reason: ExitReason::Fatal(format!(
                                        "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `ikuncodex fork` to choose from existing sessions."
                                    )),
                                });
                            }
                        }
                ...
                match resolve_session_thread_id(path.as_path(), /*id_str_if_uuid*/ None).await {
                    ...
                    None => {
                        ...
                        return Ok(AppExitInfo {
                            ...
                            update_action: None,
                            exit_reason: ExitReason::Fatal(format!(
                                "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `ikuncodex resume` to choose from existing sessions."
                            )),
                        });
                    }
                }
