...
//
// 编号（如：1）：修改
// 主要修改内容：将传统 TUI 中模型选择与会话恢复提示里的旧命令替换为 ikuncodex。
// 修改目的：保持两套 TUI 文案一致，避免后续切链路或跑测试时再次回退到旧命令提示。
//
...
impl ChatWidget {
    ...
    pub(crate) fn handle_exec_end_now(&mut self, ev: ExecCommandEndEvent) {
        // Expanded command details are only for temporary inspection. Any
        // command lifecycle transition should restore the default compact
        // single-line summary unless the user presses Ctrl and O again.
    ...
    pub(crate) fn handle_exec_begin_now(&mut self, ev: ExecCommandBeginEvent) {
        ...
        self.bottom_pane.ensure_status_indicator();
        // Each new command starts collapsed by default. Expanded output is a
        // temporary opt-in view toggled by Ctrl and O.
        let _was_expanded = crate::exec_cell::collapse_exec_output_details();
    ...
    fn update_statusline_data(&mut self) {
        ...
        // Use the primary "codex" rate limit snapshot, falling back to the first available.
        let snapshot = self
            .rate_limit_snapshots_by_limit_id
            .get("codex")
...
// 编号（如：1）：修改
// 主要修改内容：新增 Exec 输出详情切换方法，并在命令开始/结束时自动恢复默认折叠态，同时刷新活动执行单元的转录缓存版本。
// 修改目的：让 Ctrl+O 只作为临时查看详情的开关，避免命令完成后再次自动展开多行输出。
