    ...
    fn transcript_lines(&self, width: u16) -> Vec<Line<'static>> {
        if !exec_output_details_expanded() {
            ...
            for (i, call) in self.iter_calls().enumerate() {
                ...
                compact_lines.push(Self::compact_command_line(
                    call,
                    width,
                    bullet,
                    title,
...
impl ExecCell {
    ...
    fn command_display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if !exec_output_details_expanded() {
            return vec![Self::compact_command_line(
                call,
                width,
                bullet,
                title,
    ...
    fn compact_command_line(
        ...
        truncate_line_with_ellipsis_if_overflow(line, width.max(1) as usize)
    }
...
mod tests {
    ...
    fn command_display_defaults_to_single_compact_line() {
        ...
        let cell = ExecCell::new(call, false);
        let rendered = cell.command_display_lines(80);

        assert_eq!(
            rendered.len(),
            1,
            "expected a single compact line by default"
        );
        let blob = rendered
            .iter()
            .map(|line| {
            ...
            .collect::<Vec<_>>()
            .join("\n");
        assert!(blob.contains("Ran printf 'hello\\nworld'"));
        assert!(blob.contains("(120ms)"));

        crate::exec_cell::set_exec_output_details_expanded(previous);
    }

    #[test]
    fn command_display_expanded_mode_restores_output_block() {
        ...
        assert!(
            rendered.len() > 1,
            "expected expanded mode to restore detail rows"
        );
    ...
    #[test]
    fn transcript_lines_default_to_compact_command_summary() {
        let previous = crate::exec_cell::set_exec_output_details_expanded(false);
        let call = ExecCall {
            call_id: "call-id".to_string(),
            command: vec!["bash".into(), "-lc".into(), "printf 'hello\\nworld'".into()],
...
// 编号（如：1）：修改
// 主要修改内容：Exec 单元新增默认单行折叠渲染，并保留可切换回原详细渲染的分支与测试。
// 修改目的：减少主聊天区被命令输出刷屏，同时允许通过快捷键临时查看原本的详细状态。
