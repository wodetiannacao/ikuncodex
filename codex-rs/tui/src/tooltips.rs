...
const PAID_TOOLTIP: &str = "*New* Try the **Codex App** with 2x rate limits until *April 2nd*. Run 'ikuncodex app' or visit https://chatgpt.com/codex?app-landing-page=true";
const PAID_TOOLTIP_WINDOWS: &str = "*New* Try the **Codex App**, now available on **Windows**, with 2x rate limits until *April 2nd*. Run 'ikuncodex app' or visit https://chatgpt.com/codex?app-landing-page=true";
...
const OTHER_TOOLTIP: &str = "*New* Build faster with the **Codex App**. Run 'ikuncodex app' or visit https://chatgpt.com/codex?app-landing-page=true";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("ikuncodex app") {
                return false;
            }
            true
        })
...
//
// 编号（如：1）：修改
// 主要修改内容：将传统 TUI 启动提示中的 codex app/codex resume 文案统一改为 ikuncodex。
// 修改目的：防止用户从 tooltip 里复制到已经失效的旧命令。
//
