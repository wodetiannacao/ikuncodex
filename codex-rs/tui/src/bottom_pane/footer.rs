...
///
/// Note: StatusLine (CxLine) displays "? for shortcuts", so show_shortcuts_hint
/// is ignored in ComposerEmpty mode.
fn footer_from_props_lines(
    ...
    match props.mode {
        ...
        FooterMode::ComposerEmpty => {
            // StatusLine (CxLine) shows "? for shortcuts", so we skip Shortcuts hint
