...
impl HistoryCell for UserHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Add a slim top/bottom padding line so the gray background reads like
        // a compact bubble instead of only tinting the text rows themselves.
...
mod tests {
    ...
    fn user_history_cell_trims_trailing_blank_message_lines() {
        ...
        let trailing_blank_count = rendered
            ...
            .count();
        // cometix: 0 because top/bottom blank lines with background color are removed
...
// 编号（如：1）：修改
// 主要修改内容：用户消息单元重新加入顶部和底部留白，使灰底形成更完整的块级包裹效果。
// 修改目的：提升用户消息高亮的可辨识度，并让聊天正文在视觉上更有呼吸感。
