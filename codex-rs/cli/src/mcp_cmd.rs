...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `ikuncodex mcp login {name}` to login."
        ),
    }
...
//
// 编号（如：1）：修改
// 主要修改内容：将 MCP OAuth 登录提示中的命令替换为 ikuncodex mcp login。
// 修改目的：保证接入 MCP 服务时的补救提示与新 CLI 命令名一致。
//
