...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `ikuncodex login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | ikuncodex login --with-api-key`."
        );
        std::process::exit(1);
    }
...
//
// 编号（如：1）：修改
// 主要修改内容：将 CLI 登录流程中的设备码登录与 API Key 登录提示命令统一改为 ikuncodex login。
// 修改目的：确保用户首次登录时能直接复制程序提示完成认证，而不是撞到旧命令。
//
