...
#[clap(
    ...
    // The native Rust executable inside the npm package still keeps the
    // `codex` filename, but end users install and invoke the CLI via the
    // `ikuncodex` wrapper command. Pin the help output to that launcher name
    // so published help text matches what users actually type.
    bin_name = "ikuncodex",
    override_usage = "ikuncodex [OPTIONS] [PROMPT]\n       ikuncodex [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
//
// 编号（如：1）：修改
// 主要修改内容：将 CLI 帮助和退出提示测试里的旧命令样例统一改为 ikuncodex。
// 修改目的：保证面向用户的帮助文本、退出提示与测试期望都围绕新命令名收口。
//
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | ikuncodex login --with-api-key`)"
    )]
    with_api_key: bool,
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | ikuncodex login --with-api-key`."
                        );
                        std::process::exit(1);
...
mod tests {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run ikuncodex resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run ikuncodex resume my-thread".to_string(),
            ]
        );
    }
