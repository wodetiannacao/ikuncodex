...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'ikuncodex login' to sign in with ChatGPT, then re-run 'ikuncodex cloud'."
            );
            std::process::exit(1);
        }
    };
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `ikuncodex cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `ikuncodex cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("ikuncodex cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
//
// 编号（如：1）：修改
// 主要修改内容：将 cloud 任务相关的用户提示命令统一为 ikuncodex cloud / ikuncodex login。
// 修改目的：保证云任务链路里的报错与分页提示可直接复制执行，不会把用户带回旧命令。
//
