...
//
// 编号（如：1）：修改
// 主要修改内容：将 MCP 未登录错误提示中的补救命令统一为 ikuncodex mcp login。
// 修改目的：避免用户根据核心层错误提示去执行已经不存在的旧命令。
//
...
fn mcp_init_error_display(
        ...
        && http_headers.as_ref().map(HashMap::is_empty).unwrap_or(true)
    {
        ...
        format!(
            "The {server_name} MCP server is not logged in. Run `ikuncodex mcp login {server_name}`."
        )
