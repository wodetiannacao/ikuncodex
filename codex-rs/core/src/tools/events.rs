    ...
    pub async fn emit(&self, ctx: ToolEventCtx<'_>, stage: ToolEventStage) {
        match (self, stage) {
            ...
            (
                ...
                if let Err(err) = ctx
                    .session
                    .record_turn_checkpoint_patch_begin(ctx.turn, changes)
                    .await
                {
                    ctx.session
                        .notify_background_event(ctx.turn, format!("代码检查点记录失败：{err}"))
                        .await;
                }
                ctx.session
                    .send_event(
...
async fn emit_patch_end(
    ...
    if let Err(err) = ctx
        .session
        .record_turn_checkpoint_patch_end(ctx.turn, &changes)
        .await
    {
        ctx.session
            .notify_background_event(ctx.turn, format!("代码检查点收集失败：{err}"))
            .await;
    }
...
// 编号（如：1）：修改
// 主要修改内容：在 apply_patch 的开始和结束阶段接入代码检查点采集，并在失败时提示后台消息。
// 修改目的：确保 Codex 直接改动的文件能被记录下来，供后续非 Git 回退使用。
