...
use crate::turn_checkpoints::FileSnapshot;
use crate::turn_checkpoints::RollbackPlan;
use crate::turn_checkpoints::TurnCheckpointError;
use crate::turn_checkpoints::TurnCheckpointManager;
use crate::turn_checkpoints::touched_paths;
...
pub(crate) struct Session {
    ...
    turn_checkpoints: Mutex<TurnCheckpointManager>,
...
impl Session {
    ...
    async fn new(
        ...
        let sess = Arc::new(Session {
            ...
            services,
            js_repl,
            turn_checkpoints: Mutex::new(TurnCheckpointManager::default()),
    ...
    pub(crate) async fn begin_turn_checkpoint(&self, turn_id: String) {
        let mut guard = self.turn_checkpoints.lock().await;
        guard.begin_turn(turn_id);
    }

    pub(crate) async fn finish_turn_checkpoint(&self, turn_context: &TurnContext, turn_id: &str) {
        let result = {
            let mut guard = self.turn_checkpoints.lock().await;
            guard.finish_turn(turn_id)
        };
        if let Err(err) = result {
            self.notify_background_event(turn_context, format!("代码检查点收尾失败：{err}"))
                .await;
        }
    }

    pub(crate) async fn record_turn_checkpoint_patch_begin(
        &self,
        turn_context: &TurnContext,
        changes: &HashMap<PathBuf, FileChange>,
    ) -> Result<(), TurnCheckpointError> {
        let paths = touched_paths(turn_context.cwd.to_path_buf().as_path(), changes);
        let snapshots = capture_file_snapshots(&paths).await?;
        let mut guard = self.turn_checkpoints.lock().await;
        match guard.capture_before(&turn_context.sub_id, snapshots) {
            Ok(()) => Ok(()),
            Err(TurnCheckpointError::MissingActiveTurn { .. })
            | Err(TurnCheckpointError::TurnMismatch { .. }) => Ok(()),
            Err(err) => Err(err),
        }
    }

    pub(crate) async fn record_turn_checkpoint_patch_end(
        &self,
        turn_context: &TurnContext,
        changes: &HashMap<PathBuf, FileChange>,
    ) -> Result<(), TurnCheckpointError> {
        ...
        let snapshots = capture_file_snapshots(&paths).await?;
        let mut guard = self.turn_checkpoints.lock().await;
        match guard.capture_after(&turn_context.sub_id, snapshots) {
            Ok(()) => Ok(()),
            Err(TurnCheckpointError::MissingActiveTurn { .. })
            | Err(TurnCheckpointError::TurnMismatch { .. }) => Ok(()),
            Err(err) => Err(err),
    ...
    pub(crate) async fn rollback_turn_checkpoints(
        &self,
        requested_turns: u32,
        restore_files: bool,
        ...
        if !restore_files {
            let mut guard = self.turn_checkpoints.lock().await;
            guard.discard_last_turns(requested_turns);
        ...
        let plan = {
            let guard = self.turn_checkpoints.lock().await;
            guard.plan_rollback(requested_turns)?
        };
        restore_checkpoint_plan(&plan).await?;

        {
            let mut guard = self.turn_checkpoints.lock().await;
            guard.commit_rollback(requested_turns)?;
        }
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        let should_exit = async {
            match sub.op.clone() {
                ...
                Op::BacktrackRollback { num_turns, scope } => {
                    handlers::backtrack_rollback(&sess, sub.id.clone(), num_turns, scope).await;
                    false
                }
...
mod handlers {
    ...
    use codex_protocol::protocol::BacktrackScope;
    use codex_protocol::protocol::CodexErrorInfo;
    ...
    pub async fn thread_rollback(sess: &Arc<Session>, sub_id: String, num_turns: u32) {
        match prepare_thread_rollback(sess, sub_id, num_turns).await {
            Ok(prepared) => apply_prepared_thread_rollback(sess, prepared).await,
            Err(event) => sess.send_event_raw(event).await,
        }
    }

    pub async fn backtrack_rollback(
        sess: &Arc<Session>,
        sub_id: String,
        num_turns: u32,
        scope: BacktrackScope,
    ) {
        let rollback_sub_id = sub_id.clone();
        let prepared = if scope.includes_conversation() {
            match prepare_thread_rollback(sess, sub_id, num_turns).await {
            ...
            if let Some(event) = validate_thread_rollback_request(sess, &sub_id, num_turns).await {
                sess.send_event_raw(event).await;
                return;
            }
            None
        ...
        let turn_context = match prepared.as_ref() {
            Some(prepared) => Arc::clone(&prepared.turn_context),
            None => sess.new_default_turn_with_sub_id(rollback_sub_id).await,
        };

        match sess
            .rollback_turn_checkpoints(num_turns, scope.includes_files())
            .await
        {
            Ok(plan) => {
                if scope.includes_files() {
                    sess.notify_background_event(
                        turn_context.as_ref(),
                        format!(
                            "代码回退完成：已处理最近 {num_turns} 轮，恢复 {} 个文件。",
        ...
        if let Some(prepared) = prepared {
            apply_prepared_thread_rollback(sess, prepared).await;
        }
    }

    async fn prepare_thread_rollback(
        sess: &Arc<Session>,
        sub_id: String,
        num_turns: u32,
    ) -> Result<PreparedThreadRollback, Event> {
        if let Some(event) = validate_thread_rollback_request(sess, &sub_id, num_turns).await {
            return Err(event);
        }
        ...
        let rollout_path = {
            ...
            let Some(recorder) = recorder else {
                return Err(thread_rollback_error_event(
    ...
    async fn apply_prepared_thread_rollback(sess: &Arc<Session>, prepared: PreparedThreadRollback) {
        sess.persist_rollout_items(&[RolloutItem::EventMsg(prepared.rollback_msg.clone())])
            .await;
        sess.flush_rollout().await;
        sess.apply_rollout_reconstruction(
            prepared.turn_context.as_ref(),
            prepared.replay_items.as_slice(),
        )
        .await;
        sess.recompute_token_usage(prepared.turn_context.as_ref())
            .await;

        sess.deliver_event_raw(Event {
            id: prepared.turn_context.sub_id.clone(),
            msg: prepared.rollback_msg,
        })
        .await;
    }

    async fn validate_thread_rollback_request(
        sess: &Arc<Session>,
        sub_id: &str,
        num_turns: u32,
    ) -> Option<Event> {
    ...
    fn thread_rollback_error_event(sub_id: String, message: String) -> Event {
        Event {
            id: sub_id,
            msg: EventMsg::Error(ErrorEvent {
                message,
...
pub(crate) async fn run_turn(
    ...
    let checkpoint_turn_id = (!input.is_empty()).then(|| turn_context.sub_id.clone());
    if let Some(turn_id) = checkpoint_turn_id.clone() {
        sess.begin_turn_checkpoint(turn_id).await;
    }
    ...
    if let Some(turn_id) = checkpoint_turn_id.as_deref() {
        sess.finish_turn_checkpoint(turn_context.as_ref(), turn_id)
            .await;
    }
...
) -> Result<HashMap<PathBuf, FileSnapshot>, TurnCheckpointError> {
    let mut snapshots = HashMap::new();
    for path in paths {
        let snapshot = read_file_snapshot(path).await?;
        snapshots.insert(path.clone(), snapshot);
    }
    Ok(snapshots)
}

async fn restore_checkpoint_plan(plan: &RollbackPlan) -> Result<(), TurnCheckpointError> {
    for record in plan.records.iter().rev() {
        let mut touched_paths = record
            .before
            .keys()
        ...
        for path in &touched_paths {
            let expected_after = record
                .after
                .get(path)
                .cloned()
                .unwrap_or_else(FileSnapshot::missing);
            let actual = read_file_snapshot(path).await?;
            if actual != expected_after {
                return Err(TurnCheckpointError::Conflict { path: path.clone() });
            }
        }
        ...
        for path in touched_paths {
            let before = record
                ...
                .cloned()
                .unwrap_or_else(FileSnapshot::missing);
            write_file_snapshot(&path, &before).await?;
        }
    }
...
async fn read_file_snapshot(path: &Path) -> Result<FileSnapshot, TurnCheckpointError> {
    match tokio::fs::read_to_string(path).await {
        ...
        Err(err) => Err(TurnCheckpointError::Io {
...
async fn write_file_snapshot(
    ...
    if snapshot.existed {
        if let Some(parent) = path.parent() {
            tokio::fs::create_dir_all(parent)
                .await
                .map_err(|err| TurnCheckpointError::Io {
                    path: parent.to_path_buf(),
                    message: err.to_string(),
                })?;
        }
        tokio::fs::write(path, snapshot.content.as_bytes())
            .await
            .map_err(|err| TurnCheckpointError::Io {
                path: path.to_path_buf(),
                message: err.to_string(),
    ...
    {
        return Err(TurnCheckpointError::Io {
            path: path.to_path_buf(),
            message: err.to_string(),
        });
    }
...
mod tests;
// 编号（如：1）：修改
// 主要修改内容：接入会话内代码检查点、backtrack_rollback 新操作，以及按范围执行的代码/对话回退流程。
// 修改目的：让 Codex 在不依赖 Git 的前提下，按用户消息轮次回退自己修改过的代码，并与现有 thread rollback 联动。
