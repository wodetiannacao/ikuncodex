...
use codex_protocol::protocol::BacktrackScope as CoreBacktrackScope;
...
impl BacktrackScope {
    pub fn to_core(self) -> CoreBacktrackScope {
        match self {
            Self::ConversationOnly => CoreBacktrackScope::ConversationOnly,
            Self::FilesOnly => CoreBacktrackScope::FilesOnly,
            Self::ConversationAndFiles => CoreBacktrackScope::ConversationAndFiles,
        }
    }
}

impl From<CoreBacktrackScope> for BacktrackScope {
    fn from(value: CoreBacktrackScope) -> Self {
        match value {
            CoreBacktrackScope::ConversationOnly => Self::ConversationOnly,
            CoreBacktrackScope::FilesOnly => Self::FilesOnly,
            CoreBacktrackScope::ConversationAndFiles => Self::ConversationAndFiles,
        }
    }
}
...
pub struct ThreadRollbackParams {
    ...
    ///
    /// When `scope` is omitted, this only modifies the thread's history and does not revert
    /// local file changes that have been made by the agent.
    pub num_turns: u32,
    /// Optional rollback scope for non-Git backtrack flows.
    ///
    /// Omitting this preserves the legacy behavior of rolling back conversation history only.
    #[serde(default)]
    pub scope: Option<BacktrackScope>,
}
...
/*
   编号（如：1）：修改
   主要修改内容：新增 app-server 协议层 BacktrackScope，并为 ThreadRollbackParams 增加可选 scope。
   修改目的：让 app-server 能表达非 Git 的对话/代码回退范围，而不只支持历史回滚。
