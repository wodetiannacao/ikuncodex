...
impl CodexMessageProcessor {
    ...
    async fn thread_rollback(
        ...
        let rollback_op = match scope {
            Some(scope) => Op::BacktrackRollback {
                num_turns,
                scope: scope.to_core(),
            },
            None => Op::ThreadRollback { num_turns },
        };

        if let Err(err) = self
            .submit_core_op(&request_id, thread.as_ref(), rollback_op)
            .await
        {
...
/*
   编号（如：1）：修改
   主要修改内容：thread_rollback 请求处理新增 scope 分发，可映射到核心层 BacktrackRollback。
   修改目的：让 app-server 在收到带范围的回退请求时，真正触发对话/代码联动回退。
